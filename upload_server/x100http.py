...
class X100HTTPServer(BaseHTTPRequestHandler):
    ...
    def _handle_body(self, line):
        try:
            self.__class__.routers_upload[file_path_without_query_string][
    ...
    def do_POST(self):
        ...
        file_path_without_query_string = file_path[0]
        ...
        if post_type == "multipart/form-data":
            ...
            try:
                response = self.__class__.routers_upload[
                    file_path_without_query_string]['fn_del'](req).encode()
            except:
                self.send_error(500)
            else:
            ...
            if file_path_without_query_string in self.__class__.routers_post:
                req = dict()
                req['remote_ip'] = self.address_string()
                ...
                try:
                    response = self.__class__.routers_post[
                        file_path_without_query_string](req).encode()
                except:
                    self.send_error(500)
                else:
                    self.send_response(200)
    ...
    def do_GET(self):
        ...
        file_path_without_query_string = self.path.split('?')[0]
        file_path = self.path.split('?')
        file_path_without_query_string = file_path[0]

        if file_ext in self.__class__.static_file_ext:
            if os.path.exists(file_basename):
                ...
                self.send_error(404)
        elif file_path_without_query_string in self.__class__.routers_get:
            req = dict()
            req['remote_ip'] = self.address_string()
            req['body'] = ""
            req['query_string'] = ""
